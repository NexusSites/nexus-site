...
export default function Hero() {
  ...
  const DIAG_DEG = 40.3; // ~Math.atan2(1.70, 2.00) in degrees
  const headingOpacity = clamp01((progress - 0.42) / 0.06);
  const straightenT = clamp01((progress - 0.50) / 0.15);
  const headingRotation = DIAG_DEG * (1 - straightenT);
  const headingScale = 0.35 + 0.65 * clamp01((progress - 0.50) / 0.18);
      ...
      <div className="sticky top-0 h-screen overflow-hidden">
        ...
        <h2
          ...
          style={{
            ...
            transform: `translate(-50%, -50%) rotate(${headingRotation}deg) scale(${headingScale})`,
